...
public class EPOS implements Interface {
    ...
    public void printReceipt(){
        ...
        Query qu = session.createQuery("from Stock s where s.id = :results");
        arrayResults = qu.getResultList();

        session.getTransaction().commit();
        int n = arrayResults.size();



        String s = String.format("Item Price", "%-15s %5s %10s\n");
        String s1 = String.format("---- -----","%-15s %5s %10s\n");
        System.out.print(s + s1);

        Transactions trans = session.get(Transactions.class, transactionID);
        double total = trans.getMoney_given() - trans.getChange_returned();




        for(int i=0; i<n; i++){
            System.out.print(String.format(arrayResults.get(i).getName() + arrayResults.get(i).getCost(),"%-15s %5s %10s\n"));
        }

        String lastline = String.format("Total " + total, "%-15s %5s %10s\n");
        System.out.print(s1 + lastline);
        System.out.print(String.format("Paid " + trans.getMoney_given(),"%-15s %5s %10s\n"));
        System.out.print(String.format("Change " + trans.getChange_returned(),"%-15s %5s %10s\n"));
